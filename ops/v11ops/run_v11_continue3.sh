...
while pgrep -u mzjiang -f run_v11_formal3.sh > /dev/null; do sleep 60; done
L=$G/run_v11_formal3.log
...
for N in 5 10 15 20 25 30; do
  bash $G/start_servers3.sh > $G/servers_check.log 2>&1 || { echo "STOP: servers not available"; tail -5 $G/servers_check.log; exit 1; }
