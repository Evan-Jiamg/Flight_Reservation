...
[ -d $C ] || { echo "pend_v10 missing"; exit 1; }
if pgrep -u mzjiang -f run_v10_formal.sh > /dev/null; then echo "a v10 run is active: not touching it"; exit 1; fi
cd $C && chmod u+w *.py && echo "$PAYLOAD_B64" | base64 -d | tar xzf - || exit 1
chmod u+w LOCAL_SHA256 SHA256SUMS 2>/dev/null
...
0dbc60b5d9ab234d15010e90a635291a2c50f87ff964bb2e470a67ed3d401ee1  test_rl_controllers.py
...
SHAEOF
if ! sha256sum -c --quiet LOCAL_SHA256; then echo "SHA MISMATCH vs local worktree: not launching"; chmod a-w *.py; exit 1; fi
sha256sum *.py > SHA256SUMS && chmod a-w *.py
echo "tests added; $(wc -l < LOCAL_SHA256) files match the local worktree"
setsid nohup bash $G/run_v10_formal.sh > $G/run_v10_formal.log 2>&1 < /dev/null &
echo "formal gate run relaunched"
