# Start OUR two vLLM servers on GPU0 (user-approved layout 2026-09-26):
...
# Both bound to 127.0.0.1 (runtime LoRA loading must never be reachable from outside).
...
echo "=== GPUs before"; nvidia-smi --query-gpu=index,memory.used,memory.total --format=csv,noheader
up() { curl -s -m 5 http://127.0.0.1:$1/v1/models | grep -q "$2"; }
wait_up() { for i in $(seq 1 90); do up $1 $2 && return 0; sleep 10; done; return 1; }
...
echo "gpt-oss launched (0.78)"
if ! wait_up 8029 gpt-oss-120b; then echo "ABORT: gpt-oss did not come up"; tail -15 $R/serve.log | cut -c1-250; exit 1; fi
fi
echo "gpt-oss up"
...
echo "planner vLLM launched (0.15)"
if ! wait_up 8031 planner-base; then echo "ABORT: planner vLLM did not come up"; tail -15 $Q/serve.log | cut -c1-250; exit 1; fi
fi
echo "planner vLLM up"
curl -s -m 5 http://127.0.0.1:8031/v1/models | head -c 300; echo
echo "=== GPUs after"; nvidia-smi --query-gpu=index,memory.used,memory.total --format=csv,noheader
