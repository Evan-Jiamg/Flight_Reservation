...
timeout ${SSH_TIMEOUT:-110} ssh -o ConnectTimeout=20 -o BatchMode=yes "$HOST" "export PAYLOAD_B64='$P'; mkdir -p /tmp2/mzjiang_usersim; F=/tmp2/mzjiang_usersim/.cc_remote_\$\$.sh; echo $S | base64 -d > \$F; bash \$F; rc=\$?; rm -f \$F; exit \$rc"
