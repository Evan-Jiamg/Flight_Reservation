M=/tmp2/mzjiang_usersim; G=$M/grpo_planner; C=$M/code_snapshots/prism_probe_v3
pkill -f "prism_probe_v2/run_probe_after_extract.sh" && echo "v2 waiter stopped"
...
if PYTHONNOUSERSITE=1 $PY test_probe.py 2>&1 | tee /dev/stderr | grep -q "probe ok"; then
cat > $C/run_probe_after_extract.sh <<'EOF'
G=/tmp2/mzjiang_usersim/grpo_planner; C=/tmp2/mzjiang_usersim/code_snapshots/prism_probe_v3
...
EOF
setsid nohup bash $C/run_probe_after_extract.sh > /dev/null 2>&1 < /dev/null & echo "v3 probe queued"
fi
