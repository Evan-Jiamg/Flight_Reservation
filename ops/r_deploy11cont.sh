...
if pgrep -u mzjiang -f run_v11_continue.sh > /dev/null; then echo "continuation already queued"; exit 1; fi
cat > $G/run_v11_continue.sh <<'CONTEOF'
#!/bin/bash
...
while pgrep -u mzjiang -f run_v11_formal.sh > /dev/null; do sleep 60; done
L=$G/run_v11_formal.log
...
for N in 5 10 15 20 25 30; do
  bash $G/start_servers.sh > $G/servers_check.log 2>&1 || { echo "STOP: servers not available"; tail -5 $G/servers_check.log; exit 1; }
...
CONTEOF
setsid nohup bash $G/run_v11_continue.sh > $G/run_v11_continue.log 2>&1 < /dev/null &
echo "continuation queued (waits for the gate)"
tail -3 $G/run_v11_formal.log | cut -c1-200
